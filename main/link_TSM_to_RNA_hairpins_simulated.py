...
def TSM_analysis(i, text, filename, align_dict, tree,
    ...
    results_dict["identifier"] =\
        filename.split('/')[-1].rstrip('_fpa') + '_' + str(position)
    ...
    results_dict["alignment_file"] = results_dict["identifier"] + '.fas'
    ...
    align_ts_qry =\
        str(align_dict[query_node])[int(qry_ts_start):int(qry_ts_end)]
    align_ts_ref =\
        str(align_dict[ref_node])[int(qry_ts_start):int(qry_ts_end)]
    align_orig_ref =\
        str(align_dict[ref_node])[int(ref_ts_start):int(ref_ts_end)]
    align_orig_qry =\
        str(align_dict[query_node])[int(ref_ts_start):int(ref_ts_end)]
    ...
    if not (tree&query_node).is_leaf():
        ...
        child_struct1 = structure_dir +\
            filename.split('/')[-1].rstrip('_fpa') + '_' +\
            qry_child_structure_node1 + '.db'
        child_struct2 = structure_dir +\
            filename.split('/')[-1].rstrip('_fpa') + '_' +\
            qry_child_structure_node2 + '.db'
        ...
        if not (tree&query_node).is_leaf() and flag_qry is True:
            for ch in children_structs:
                (align_ch_orig,
                 align_qrych_orig) =\
                    unusual_iupac_char(
                        align_dict[ch][int(ref_ts_start):int(ref_ts_end)],
                ...
                (align_ch_ts,
                 align_qrych_ts) =\
                    unusual_iupac_char(
                        align_dict[ch][int(qry_ts_start):int(qry_ts_end)],
                        align_dict[query_node][
    ...
    quality_info = False
    if (tree&query_node).is_leaf() and flag_qry is True and flag_ref is True:
    ...
    # Collect information from the TSM-linked changes
    # in the RNA hairpin
    if compensating > 0:
    ...
    if loop_seq_mismatch > 0 and unsymmetric_subs == 0 and\
            mismatches >= 2 and results_dict["stem_extended"] is False and\
    ...
    return (i, results_dict, quality_info,
            index_info, mutation_types, align_ts_qry_c,
            align_orig_ref_c)
...
def main():
    ...
    fpa_dir: A directory that contains fpa results file. They should be
             named as identifier_fpa, respectively. The alignment file
    ...
    mutation_output: A file where CM counts are recorded (base_pair in
                     parental stem, base pair in child node).
    database: Database in which TSMs are saved
    cpu: Number of cpus
    regions_file: A file containing high quality regions
    alignment_dir: Adirectory containing alignment files
    tree_dir: A directory containing tree files

    Output
    =======

    Creates a database to which a document saved from
    each TSM case.
    ...
    main_dir = structure_dir.split('/')[0]
    ...
    for subdir, dirs, files in os.walk(fpa_directory):
        for file in files:
            ...
            if filename.endswith('_fpa'):
                ...
                features_dict = get_node_db(tree,
                                            'Rfam_loops_v2',
                                            'rfam_loops')
                ...
                with open(
                        main_dir + "/clean/" + identifier + '.fas', 'a+') as f:
                    for seq in align_dict:
                        f.write('>' + seq + '\n')
                        f.write(align_dict[seq] + '\n')

                template_switch_clusters = map_template_switch_in_same_region(
                        fpa_directory + '/' + str(file),
                        main_dir + "/clean/" + identifier + '.fas',
                        do_not_merge=True)

                # Goes through all identified TSMs in the alignment
                position_list = []
                ...
                results_objects = [pool.apply_async(
                    TSM_analysis,
                    args=(i, text, filename, align_dict,
                          tree,
                          structure_dir,
                ...
                for a in range(len(results1)):
                    ...
                    if results2[a] is True and (
                        ...
                        if results3[a] is not None and\
                                results1[a]["ts_distance"] < 9:
                            for mutation in results3[a]:
